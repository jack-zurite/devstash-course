...
Read the current feature from: @context/current-feature.md

## File Structure

current-feature.md has these actions:

- `# Current Feature` - H1 heading with feature name when active
- `## Status` - Not Started | In Progress | Complete
- `## Goasl` - Bullet points of what success looks like
...
1. Check $ARGUMENTS (after "load"):
  - If it loads like a filename (single word, no spaces): Look for `context/features/{name}.md`
...
2. Update current-feature.md:
  - Update H1 heading to include feature name (e.g., `# Curren Feature: Add Navbar`)
